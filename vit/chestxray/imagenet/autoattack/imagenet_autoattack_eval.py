"""
ChestX-ray Dataset - AutoAttack + Guided-Diffusion (ImageNet Pretrained) Defense (ViT Classifier)
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='ChestX-ray AutoAttack + Guided-Diffusion Defense (ViT)')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/vit/chestxray/imagenet/autoattack/results',
...
def get_accuracy_with_purification(model, purifier, x, y, bs=4, device=None):
    """浄化付きで精度を計算"""
    if device is None:
        device = next(model.parameters()).device
    ...
    for i in tqdm(range(n_batches), desc="Purification"):
        start_idx = i * bs
        end_idx = min((i + 1) * bs, len(x))
        x_batch = x[start_idx:end_idx].to(device)
        y_batch = y[start_idx:end_idx].to(device)
...
def get_predictions_with_purification(model, purifier, x, bs=4, device=None):
    """浄化付きで予測を取得"""
    if device is None:
        device = next(model.parameters()).device
    ...
    for i in tqdm(range(n_batches), desc="Getting predictions with purification"):
        start_idx = i * bs
        end_idx = min((i + 1) * bs, len(x))
        x_batch = x[start_idx:end_idx].to(device)
...
def main():
    ...
    clean_purified_acc = get_accuracy_with_purification(classifier_model, purifier, x_test, y_test, bs=4, device=device)
    ...
    adv_defended_acc = get_accuracy_with_purification(classifier_model, purifier, x_adv, y_test, bs=4, device=device)
    ...
    pred_adv_defended = get_predictions_with_purification(classifier_model, purifier, x_adv, bs=4, device=device)
    ...
    with open(summary_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("ChestX-ray - AutoAttack + Guided-Diffusion Defense (ViT Classifier)\n")
        f.write("="*70 + "\n\n")

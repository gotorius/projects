"""
ChestX-ray Dataset - AutoAttack + JPEG Compression Defense (ViT Classifier)
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='ChestX-ray AutoAttack + JPEG Defense (ViT)')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/vit/chestxray/jpeg/autoattack/results',
...
def main():
    ...
    defended_model = JPEGDefenseWrapper(jpeg_defense, classifier, IMAGENET_MEAN, IMAGENET_STD).to(device).eval()
    ...
    # ==================== 評価開始 ====================
    print(f"\n{'='*70}")
    print("AutoAttack + JPEG Defense Evaluation (ViT Classifier)")
    print(f"{'='*70}")
    ...
    clean_jpeg_acc = get_accuracy(defended_model, x_test, y_test, bs=args.batch_size, device=device)
    print(f"Clean accuracy (with JPEG): {clean_jpeg_acc:.4f}")
    results['clean_jpeg_acc'] = clean_jpeg_acc
    ...
    print(f"Clean accuracy (with JPEG):  {results['clean_jpeg_acc']:.4f}")
    print(f"Adversarial (no defense):    {results['adv_acc_no_defense']:.4f}")
    print(f"Adversarial (with JPEG):     {results['adv_defended_acc']:.4f}")
    ...
    cm_adv_defended = print_confusion_matrix(y_true, pred_adv_defended, "3. AutoAttack Images (with JPEG)", classes)
    
    # ==================== サンプル画像保存 ====================
    print("\nGenerating JPEG compressed samples for visualization...")
    n_samples = min(10, len(x_test))
    x_compressed = []
    for i in range(n_samples):
        x_comp = jpeg_defense(x_adv[i:i+1].to(device))
        x_compressed.append(x_comp.cpu())
    ...
    with open(summary_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("ChestX-ray - AutoAttack + JPEG Defense (ViT Classifier)\n")
        f.write("="*70 + "\n\n")
        ...
        f.write(f"Clean accuracy (with JPEG):  {results['clean_jpeg_acc']:.4f}\n")
        f.write(f"Adversarial (no defense):    {results['adv_acc_no_defense']:.4f}\n")
        f.write(f"Adversarial (with JPEG):     {results['adv_defended_acc']:.4f}\n")

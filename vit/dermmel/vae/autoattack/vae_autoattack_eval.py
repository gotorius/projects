...
def main():
    ...
    print(f"\n{'='*60}")
    print("DermMel VAE (MagNet) AutoAttack Evaluation (ViT)")
    print(f"{'='*60}")
    ...
    print(f"Attack: AutoAttack ({args.aa_version}, {args.norm}, eps={args.epsilon:.5f})")
    ...
    print(f"{'AutoAttack + VAE':<35} {adv_vae_acc:>10.4f}")
    print(f"{'='*60}")
    ...
    results = {
        ...
        'model': 'ViT-B/16',
        'attack': 'AutoAttack',
        'attack_version': args.aa_version,
        'norm': args.norm,
        'defense': 'VAE (MagNet)',
    ...
    result_path = os.path.join(args.output_dir, f'autoattack_vae_results_{timestamp}.json')
    ...
    with torch.no_grad():
        x_vae = vae.reconstruct(x_adv_cpu.to(device)).cpu()

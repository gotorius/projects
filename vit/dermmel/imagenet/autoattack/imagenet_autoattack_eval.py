"""
DermMel Dataset - AutoAttack + Guided-Diffusion (ImageNet Pretrained) Defense (ViT Classifier)
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='DermMel AutoAttack + Guided-Diffusion Defense (ViT)')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/vit/dermmel/imagenet/autoattack/results',
...
def main():
    ...
    with open(summary_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("DermMel - AutoAttack + Guided-Diffusion Defense (ViT Classifier)\n")
        f.write("="*70 + "\n\n")
        ...
        for name, cm in [("Clean (ViT Classifier)", cm_clean), 
                         ("AutoAttack (No Defense)", cm_adv_no_def),
                         ("AutoAttack (with Guided-Diffusion)", cm_adv_defended)]:
            if cm:
                f.write(f"{name}:\n")

...
def main():
    ...
    print(f"\n{'='*60}")
    print("DermMel PGD Attack + JPEG Defense Evaluation (ViT)")
    print(f"{'='*60}")
    ...
    # ========== 3. PGD敵対的画像の生成と評価 ==========
    print(f"\n[3/4] Generating PGD adversarial examples (eps={args.epsilon:.5f}, steps={args.pgd_steps})...")
    ...
    for i in tqdm(range(n_batches), desc="PGD attack"):
        ...
        x_adv_batch = pgd_attack(clf_wrapper, x_batch, y_batch, args.epsilon, args.alpha, 
                                  args.pgd_steps, device, args.random_start)
    ...
    print(f"{'PGD + JPEG':<35} {adv_jpeg_acc:>10.4f}")
    print(f"{'='*60}")
    ...
    result_path = os.path.join(args.output_dir, f'pgd_jpeg_results_{timestamp}.json')

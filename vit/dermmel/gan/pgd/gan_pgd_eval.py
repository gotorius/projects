"""
Defense-GAN Adversarial Defense Evaluation for DermMel Dataset (ViT Classifier) - PGD Attack
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='Defense-GAN Evaluation for DermMel (ViT) - PGD Attack')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/vit/dermmel/gan/pgd/results',
...
class Generator(nn.Module):
    ...
    Note: nc=3 for RGB DermMel images
    """
...
# ========== Defense-GAN Purification for DermMel ==========
class DefenseGAN:
    """
    Defense-GAN for DermMel: 敵対的画像をGANの生成器を使って浄化
...
def load_classifier(args, device):
    """ViT分類器を読み込み"""
    # ViT分類器（2クラス: NotMelanoma, Melanoma）
...
def run_pgd_attack(model, x_test, y_test, epsilon, alpha, steps, device, batch_size=32, random_start=True):
    ...
    print(f"\nRunning PGD attack: epsilon={epsilon:.4f}, alpha={alpha:.4f}, steps={steps}")
...
def main():
    ...
    # ==================== 評価開始 ====================
    print(f"\n{'='*70}")
    print("PGD Attack + Defense-GAN Evaluation (ViT Classifier)")
    print(f"{'='*70}")
    ...
    with open(summary_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("DermMel - PGD Attack + Defense-GAN (ViT Classifier)\n")
        f.write("="*70 + "\n\n")

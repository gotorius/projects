"""
DermMel Dataset - AutoAttack + Defense-GAN Defense (ViT Classifier)
...
DermMel特有の考慮:
- RGB画像（3チャンネル）→ Generator出力も3チャンネル
- 皮膚病変画像
- クラス: NotMelanoma (0), Melanoma (1)
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='Defense-GAN Evaluation for DermMel (ViT) - AutoAttack')
    ...
    parser.add_argument('--gan_ckpt', type=str,
                        default='/mnt/data1/gotou/projects/resnet/dermmel/gan/checkpoints/20260102_195110/final_model.pth',
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/vit/dermmel/gan/autoattack/results',
...
# ========== Generator for DermMel (RGB, nc=3) ==========
...
class DefenseGAN:
    ...
    def reconstruct(self, x):
        ...
        x_target = self._to_tanh_space(x)
...
def main():
    ...
    log_dir = os.path.join(args.output_dir, f"autoattack_{args.version}_eps{args.epsilon:.4f}_{timestamp}")
    ...
    print(f"{'='*70}")
    print(f"Dataset: DermMel (Skin Melanoma)")
    print(f"AutoAttack Version: {args.version}")
    ...
    # ========== 2. AutoAttack ==========
    print(f"\n[2/3] Running AutoAttack ({args.version})...")
    ...
    print(f"Attack: AutoAttack ({args.version})")
    print(f"Norm: {args.norm}, Epsilon: {args.epsilon:.4f} ({args.epsilon*255:.1f}/255)")
    print(f"Defense-GAN: rec_iters={args.rec_iters}, rec_lr={args.rec_lr}, rec_rr={args.rec_rr}")
    print(f"Note: Generator is trained on RGB images")
    print(f"-"*70)
    print(f"Clean Accuracy:              {results['clean_acc_classifier']:.4f}")
    print(f"-"*70)
    ...
    with open(summary_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("DermMel - AutoAttack + Defense-GAN (ViT Classifier)\n")
        f.write("="*70 + "\n\n")
        f.write(f"Dataset: DermMel (Skin Melanoma)\n")
        ...
        f.write(f"Clean Accuracy:              {results['clean_acc_classifier']:.4f}\n\n")

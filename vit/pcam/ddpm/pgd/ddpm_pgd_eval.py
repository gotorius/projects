"""
PCam Dataset - PGD Attack + DDPM Purification Defense (ViT Classifier)
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='PCam PGD Attack + DDPM Defense (ViT)')
    
    # PGD攻撃設定
    parser.add_argument('--epsilon', type=float, default=8/255,
                        help='PGD perturbation epsilon (pixel scale 0-1)')
    parser.add_argument('--alpha', type=float, default=2/255,
                        help='PGD step size alpha (pixel scale 0-1)')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/vit/pcam/ddpm/pgd/results',
...
class SimpleUNet(nn.Module):
    """RGB用UNet with Attention (in_ch=3)"""
...
class DDPMPurifierRGB(nn.Module):
    ...
    def purify(self, x_pixel_rgb):
        ...
        # [0,1] → [-1,1]
        x_ddpm = self.pixel_to_ddpm(x_pixel_rgb)
        ...
        # [-1,1] → [0,1]
        x_purified = self.ddpm_to_pixel(x0_hat)
...
def main():
    ...
    # ==================== 評価開始 ====================
    print(f"\n{'='*70}")
    print("PCam - PGD Attack + DDPM Defense Evaluation (ViT Classifier)")
    print(f"{'='*70}")
    ...
    pred_clean, clean_acc = get_predictions_and_accuracy(classifier_model, x_test, y_test, bs=args.batch_size, device=device)
    ...
    pred_clean_purified, clean_purified_acc = get_predictions_and_accuracy(defense_model, x_test, y_test, bs=args.batch_size, device=device)
    ...
    pred_adv_no_def, adv_acc_no_defense = get_predictions_and_accuracy(classifier_model, x_adv, y_test, bs=args.batch_size, device=device)
    ...
    pred_adv_defended, adv_defended_acc = get_predictions_and_accuracy(defense_model, x_adv, y_test, bs=args.batch_size, device=device)
    ...
    print(f"Attack: PGD, Epsilon: {args.epsilon:.4f} ({args.epsilon*255:.1f}/255), Alpha: {args.alpha:.4f}, Steps: {args.pgd_steps}")
    print(f"DDPM: start_t={args.start_t}, T_purify={args.T_purify}")
    print(f"-"*70)
    ...
    # ==================== サマリー保存 ====================
    summary_path = os.path.join(log_dir, 'results.txt')
    with open(summary_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("PCam - PGD Attack + DDPM Defense (ViT Classifier)\n")
        f.write("="*70 + "\n\n")

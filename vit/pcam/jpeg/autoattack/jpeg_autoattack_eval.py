...
def parse_args():
    ...
    parser.add_argument('--aa_version', type=str, default='standard', choices=['standard', 'plus', 'rand'])
...
def run_autoattack(model, x_test, y_test, epsilon, norm, version, device, batch_size):
    adversary = AutoAttack(model, norm=norm, eps=epsilon, version=version, verbose=True)
    adversary.seed = 42
    with torch.no_grad():
        x_adv = adversary.run_standard_evaluation(x_test.to(device), y_test.to(device), bs=batch_size)
    return x_adv.cpu()
...
def main():
    ...
    print(f"\n{'='*70}")
    print("PCam - AutoAttack + JPEG Defense (ViT)")
    print(f"{'='*70}")
    ...
    x_adv = run_autoattack(classifier_model, x_test, y_test, args.epsilon, args.norm, args.aa_version, device, args.batch_size)
    ...
    print(f"\nClean: {results['clean_acc_classifier']:.4f} | Clean+JPEG: {results['clean_acc_with_jpeg']:.4f}")
    print(f"AutoAttack: {results['adv_acc_no_defense']:.4f} | AutoAttack+JPEG: {results['adv_acc_with_jpeg']:.4f}")

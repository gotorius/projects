"""
PCam Dataset - VAE (MagNet-style) AutoAttack Evaluation (ViT Classifier)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='VAE (MagNet-style) + AutoAttack Evaluation for PCam (ViT Classifier)',
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/vit/pcam/vae/autoattack/results',
                        help='結果出力ディレクトリ')
...
# ========== AutoAttack実行 ==========
def run_autoattack(model, x_test, y_test, epsilon, norm, version, device, batch_size, seed=42):
...
def main():
    ...
    log_dir = os.path.join(args.output_dir, f"autoattack_{args.norm}_eps{args.epsilon:.4f}_{timestamp}")
    ...
    # ==================== 評価開始 ====================
    print(f"\n{'='*70}")
    print("AutoAttack + VAE Defense Evaluation (ViT Classifier)")
    print(f"{'='*70}")
    print(f"Dataset: PCam")
    print(f"Epsilon: {args.epsilon:.4f} ({args.epsilon*255:.1f}/255)")
    print(f"Norm: {args.norm}")
    print(f"Version: {args.aa_version}")
    ...
    x_adv = run_autoattack(classifier_model, x_test, y_test, args.epsilon, args.norm, 
                           args.aa_version, device, args.batch_size, args.seed)
    ...
    print(f"Attack: AutoAttack ({args.aa_version})")
    ...
    print(f"-"*70)
    print(f"Attack time: {results['attack_time']:.2f}s ({results['attack_time']/60:.1f}min)")
    print(f"{'='*70}")
    ...
    with open(summary_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("PCam - AutoAttack + VAE Defense (ViT Classifier)\n")
        f.write("="*70 + "\n\n")
        ...
        f.write(f"Attack time: {results['attack_time']:.2f}s ({results['attack_time']/60:.1f}min)\n\n")

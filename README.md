...
3. **複数の分類器での検証**: ResNet50とViT-B/16の2つの分類器アーキテクチャで評価
4. **複数の防御手法の比較**: DDPM、VAE、GAN、JPEG圧縮、ImageNet事前学習モデルを比較
5. **複数の攻撃手法への対応**: FGSM、PGD-10、AutoAttack（APGD-CE）に対する頑健性を検証
...
│       ├── gan/                    # GAN防御実験
│       └── resnet/                 # ResNet50分類器
├── vit/                            # ViT-B/16分類器を使用した実験
│   ├── chestxray/                  # 胸部X線画像（肺炎分類）
│   │   ├── ddpm/                   # DDPM防御実験
│   │   ├── jpeg/                   # JPEG圧縮防御実験
│   │   ├── vae/                    # VAE防御実験
│   │   ├── gan/                    # GAN防御実験
│   │   └── imagenet/               # ImageNet事前学習モデル防御
│   ├── dermmel/                    # 皮膚病変画像（メラノーマ分類）
│   │   ├── ddpm/                   # DDPM防御実験
│   │   ├── jpeg/                   # JPEG圧縮防御実験
│   │   ├── vae/                    # VAE防御実験
...
本研究では2つの分類器アーキテクチャを使用：
- **ResNet50**: ImageNet事前学習済み、医療画像用にファインチューニング
- **ViT-B/16**: Vision Transformer、ImageNet事前学習済み、医療画像用にファインチューニング
...
**実験設定**:
- 対象画像数: 500枚（クリーン画像で正しく分類された画像）
- DDPM: start_t=80, T_purify=50
- JPEG: quality=11
- ε=8/255 (≈0.0314)

| 攻撃手法 | Adv Acc | DDPM防御 | JPEG防御 | DDPM改善 |
|---------|---------|----------|----------|----------|
| FGSM | 64.40% | **91.60%** | 64.40% | +27.20% |
| PGD-10 | 0.00% | **90.00%** | 67.40% | +90.00% |
| AutoAttack | 0.00% | **88.60%** | 67.00% | +88.60% |
...
| 攻撃手法 | Adv Acc | DDPM防御 | JPEG防御 | DDPM改善 |
|---------|---------|----------|----------|----------|
| FGSM | 43.42% | **53.47%** | 49.53% | +10.05% |
| PGD-10 | 0.00% | **64.91%** | 49.56% | +64.91% |
| AutoAttack | 0.00% | **61.97%** | 49.56% | +61.97% |
...
| 攻撃手法 | Adv Acc | DDPM | VAE | GAN | ImageNet | JPEG |
|---------|---------|------|-----|-----|----------|------|
...
**パラメータ**:
- DDPM: start_t=280, T_purify=300
- VAE: latent_dim=256, base_ch=48
- GAN: rec_iters=150, rec_lr=0.01, rec_rr=3
- ImageNet: start_t=80, T_purify=50
- JPEG: quality=11
...
| 攻撃手法 | Adv Acc | VAE | GAN | ImageNet | JPEG |
|---------|---------|-----|-----|----------|------|
...
**パラメータ**:
- VAE: latent_dim=512, base_ch=64, RGB input
...
| 攻撃手法 | Adv Acc | DDPM | DDPM改善 |
|---------|---------|------|----------|
| FGSM | 82.60% | 78.40% | -4.20% |
| PGD-10 | 16.20% | **77.20%** | +61.00% |
| AutoAttack | 0.00% | **76.80%** | +76.80% |
...
### ChestXray における防御手法比較（ViT分類器）

| 防御手法 | FGSM | PGD-10 | AutoAttack | 平均 |
|---------|------|--------|------------|------|
| **VAE** | **91.40%** | **94.20%** | 93.60% | **93.07%** |
| **GAN** | 88.80% | 88.80% | 90.00% | 89.20% |
| **DDPM** | 85.00% | 88.00% | 88.40% | 87.13% |
| **ImageNet** | 56.00% | 71.00% | **92.40%** | 73.13% |
| **JPEG** | 51.40% | - | - | - |
| 攻撃成功 | 40.00% | 0.00% | 0.00% | - |

### 主要な知見

1. **VAEが最も効果的**: ChestXrayにおいて、VAEは全ての攻撃に対して最も高い防御精度を達成（平均93.07%）
2. **GANも高い防御性能**: VAEに次いで高い防御性能（平均89.20%）
3. **DDPMは安定した防御**: 全ての攻撃に対して安定した防御性能を発揮
4. **ImageNet事前学習モデル**: AutoAttackに対して特に効果的（92.40%）だが、FGSMには弱い
5. **JPEG圧縮の限界**: 単純なJPEG圧縮は限定的な防御効果しかない

### データセット依存性

| データセット | 最良防御手法 | 最良精度 | 特記事項 |
|-------------|-------------|---------|---------|
| **ChestXray (ViT)** | VAE | 94.20% (PGD) | グレースケール画像で高い防御効果 |
| **PCam (ViT)** | VAE | 78.00% (AutoAttack) | RGB画像でも有効 |
| **DermMel (ViT)** | DDPM | 77.20% (PGD) | 複雑な皮膚画像で有効 |
| **ChestXray (ResNet)** | DDPM | 91.60% (FGSM) | 強力な攻撃に対して高い防御 |
...
### 1. 生成モデルベース防御の有効性
- **VAE、GAN、DDPMはいずれも効果的な防御手法**
- 特にVAEはChestXrayで最高の防御性能を達成
- DDPMは強力な攻撃（PGD、AutoAttack）に対して特に効果的

### 2. 分類器アーキテクチャの影響
- **ViTはResNetと同等以上の敵対的脆弱性を示す**
- 防御手法の効果は分類器によって異なる
- VAEはViTとの組み合わせで特に高い効果

### 3. 攻撃手法への対応
- **FGSMは最も防御しやすい攻撃**
- PGDとAutoAttackは強力だが、生成モデルベース防御で80-90%以上の精度回復が可能

### 4. 医療画像AIへの示唆
- 生成モデルベース防御は医療画像AIのセキュリティ向上に有望
- データセットの特性に応じた防御手法の選択が重要
- 複数の防御手法の組み合わせがさらなる改善の余地

"""
Defense-GAN Adversarial Defense Evaluation for PCam Dataset - FGSM Attack (v4 Improved)
...
実行例:
python gan_fgsm_eval_v4.py --epsilon 0.031 --use_defense
python gan_fgsm_eval_v4.py --epsilon 0.031 --rec_iters 200 --rec_rr 5
python gan_fgsm_eval_v4.py --epsilon 0.031 --use_defense --quick_test
"""
...
# ========== 引数 ==========
def parse_args():
    parser = argparse.ArgumentParser(description='Defense-GAN Evaluation v4 - FGSM Attack')
    ...
    parser.add_argument('--rec_iters', type=int, default=200,
                        help='L-BFGS iterations (fewer needed than Adam)')
...
def main():
    ...
    log(f"\n{'='*70}")
    log("FGSM Attack + Defense-GAN v4 Evaluation (PCam)")
    log(f"{'='*70}")

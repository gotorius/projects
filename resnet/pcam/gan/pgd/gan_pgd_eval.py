"""
Defense-GAN Adversarial Defense Evaluation for PCam Dataset - PGD Attack
...
python gan_pgd_eval.py --epsilon 0.031 --pgd_steps 20 --pgd_alpha 0.008
"""
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='Defense-GAN Evaluation - PGD Attack')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/pcam/gan/pgd/results',
...
def main():
    ...
    write_and_print(f"\n{'='*70}")
    write_and_print("PGD Attack + Defense-GAN Evaluation")
    write_and_print(f"{'='*70}")
    write_and_print(f"Attack: PGD")
    write_and_print(f"  - Epsilon: {args.epsilon:.4f} ({args.epsilon*255:.1f}/255)")
    write_and_print(f"  - Alpha: {args.pgd_alpha:.4f} ({args.pgd_alpha*255:.1f}/255)")
    write_and_print(f"  - Steps: {args.pgd_steps}")
    ...
    # 3. PGD攻撃
    write_and_print(f"\n[3/4] Running PGD attack (eps={args.epsilon:.4f}, alpha={args.pgd_alpha:.4f}, steps={args.pgd_steps})...")
    ...
    write_and_print(f"{'='*70}")
    write_and_print(f"Attack: PGD (eps={args.epsilon:.4f}, alpha={args.pgd_alpha:.4f}, steps={args.pgd_steps})")
    write_and_print(f"Defense: Defense-GAN (enabled={args.use_defense})")
    write_and_print(f"-"*70)

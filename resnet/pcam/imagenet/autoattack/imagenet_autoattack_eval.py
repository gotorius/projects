"""
PCam Dataset - AutoAttack + Guided-Diffusion (ImageNet Pretrained) Defense

ImageNet事前学習済み拡散モデルを用いた敵対的防御評価

実行例:
python imagenet_autoattack_eval.py --epsilon 0.031 --start_t 80 --t_purify 50
"""
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='PCam AutoAttack + Guided-Diffusion Defense')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/pcam/imagenet/autoattack/results',
...
class GuidedDiffusionPurifier(nn.Module):
    ...
    入力: RGB画像 [0,1] (224x224)
    出力: 浄化後のRGB画像 [0,1] (224x224)
    """
...
# ========== AutoAttack攻撃 ==========
def autoattack_attack(model, x, y, epsilon, device, norm='Linf', version='standard'):
    """AutoAttack攻撃"""
    model.eval()
...
def main():
    ...
    write_and_print(f"\n{'='*70}")
    write_and_print("AutoAttack + Guided-Diffusion (ImageNet) Defense Evaluation")
    write_and_print(f"{'='*70}")
    write_and_print(f"Attack: AutoAttack ({args.attack_version}), Epsilon: {args.epsilon:.4f} ({args.epsilon*255:.1f}/255)")
    ...
    write_and_print("This may take a while...")
    start_time = time.time()
    x_adv = autoattack_attack(
        normalized_classifier, x_test, y_test, 
        args.epsilon, device, args.norm, args.attack_version
    )
    x_adv = x_adv.cpu()

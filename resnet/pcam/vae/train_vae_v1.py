"""
VAE (MagNet-style) Training Script for PCam (PatchCamelyon) Dataset
...
PCam データセット情報:
============================================================
- 画像サイズ: 96x96 (オリジナル) → 224x224 にリサイズ可能
- チャンネル: RGB (3チャンネル)
- クラス: 2クラス (転移なし/転移あり)
- 用途: リンパ節転移の検出
...
実行例:
============================================================
python train_vae_v1.py --epochs 300 --batch_size 16 --gpu 0
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='Train VAE for PCam')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/pcam/vae/checkpoints',
...
class Encoder(nn.Module):
    ...
    def __init__(self, img_channels=3, base_ch=48, latent_dim=256):
        super().__init__()
...
class Discriminator(nn.Module):
    """Lightweight Discriminator for RGB images"""
...
def train(args):
    ...
    print(f"\n{'='*70}")
    print(f"Starting VAE training for PCam dataset...")
    print(f"{'='*70}")

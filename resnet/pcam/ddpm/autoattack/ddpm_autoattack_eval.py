"""
DDPM-based Adversarial Defense Evaluation for PCam Dataset - AutoAttack

実行例:
python ddpm_autoattack_eval.py --epsilon 0.031 --version standard
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='DDPM Defense - AutoAttack')
    
    # 攻撃設定
    parser.add_argument('--epsilon', type=float, default=8/255, help='AutoAttack epsilon')
...
def main():
    ...
    print(f"\n{'='*70}")
    print("AutoAttack + DDPM Defense Evaluation")
    print(f"{'='*70}")
    print(f"Attack: AutoAttack ({args.version}), Epsilon: {args.epsilon:.4f}")
    ...
    print("\n[3/4] AutoAttack...")
    # 正規化を含むモデル
    model_normalized = NormalizedClassifier(classifier, IMAGENET_MEAN, IMAGENET_STD)
    
    # AutoAttack
    adversary = AutoAttack(model_normalized, norm='Linf', eps=args.epsilon, version=args.version,
                           device=device, log_path=os.path.join(log_dir, 'autoattack.log'))
    ...
    print("FINAL RESULTS")
    print(f"{'='*70}")
    print(f"Attack: AutoAttack ({args.version}, ε={args.epsilon:.4f})")
    print(f"-"*70)

"""
ChestXray (肺炎分類) - AutoAttack攻撃 + JPEG圧縮防御検証スクリプト
FGSM/PGD版と同一フォーマットで統計/CSV/トリプレットを出力
"""
import os
import io
import torch
...
out_dir = f'/mnt/data1/gotou/projects/chestxray/jpeg/autoattack/results_q{JPEG_QUALITY}_autoattack'
...
print(f"\n======================================")
print("Starting AutoAttack + JPEG purification evaluation (ChestXray)")
print("[Evaluation policy] Use only samples correctly classified by the clean model")
print("======================================")

for batch_idx, (x_norm, y) in enumerate(tqdm(test_loader, desc='Eval (AutoAttack->JPEG)')):
...
if total == 0:
    ...
    print("\n======================================")
    print("Results (ChestXray AutoAttack + JPEG)")
    print("======================================")
    ...
    with open(summary_txt, 'w') as f:
        f.write('=' * 70 + '\n')
        f.write('ChestXray - AutoAttack + JPEG Purification Summary\n')
        f.write('=' * 70 + '\n\n')

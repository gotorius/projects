"""
ChestX-ray Dataset - ResNet50分類器の攻撃耐性評価
==================================================
...
使用法:
    python evaluate_resnet50_attacks.py --gpu 0
    python evaluate_resnet50_attacks.py --batch_size 16 --gpu 1
    python evaluate_resnet50_attacks.py --epsilon 0.03137 --gpu 0
"""
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='ChestX-ray ResNet50 Adversarial Attack Evaluation')
    ...
    parser.add_argument('--clf_ckpt', type=str,
                        default='/mnt/data1/gotou/projects/resnet/chestxray/resnet/resnet50_best.pth',
                        help='ResNet50 Classifier checkpoint path')
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/resnet/chestxray/resnet/attack_eval_results',
...
# ========== ResNet50分類器ラッパー ==========
class ResNet50ClassifierWrapper(nn.Module):
    """ResNet50分類器のラッパー
    入力: [0,1]のRGB画像
    出力: 2クラスロジット
    """
...
# ========== モデル読み込み ==========
def load_resnet50_classifier(ckpt_path, device):
    """ResNet50分類器を読み込み"""
    print(f"\nLoading ResNet50 classifier from: {ckpt_path}")
    
    # ResNet50（2クラス: NORMAL, PNEUMONIA）
...
def main():
    ...
    # モデル読み込み
    classifier = load_resnet50_classifier(args.clf_ckpt, device)
    model = ResNet50ClassifierWrapper(classifier).to(device)
    model.eval()
    ...
    print("\n" + "="*70)
    print("Starting Adversarial Attack Evaluation (ResNet50)")
    print("="*70)

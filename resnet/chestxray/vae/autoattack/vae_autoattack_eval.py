"""
VAE (MagNet-style) AutoAttack Evaluation Script for ChestX-ray Dataset
...
実行例:
python vae_autoattack_eval.py --vae_ckpt ../checkpoints/best_model.pth --gpu 0
"""
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='VAE (MagNet) AutoAttack Evaluation')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/chestxray/vae/autoattack/results',
...
def main():
    ...
    print(f"\n{'='*70}")
    print("AutoAttack + VAE (MagNet) Defense Evaluation")
    print(f"{'='*70}")
    ...
    adversary = AutoAttack(classifier_model, norm='Linf', eps=args.epsilon,
                           version=args.attack_version, device=device)
    ...
    with open(os.path.join(log_dir, 'results.txt'), 'w') as f:
        f.write("="*70 + "\n")
        f.write("AutoAttack + VAE (MagNet) Defense Evaluation Results\n")
        f.write("="*70 + "\n\n")

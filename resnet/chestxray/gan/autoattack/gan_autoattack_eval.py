"""
Defense-GAN AutoAttack Evaluation Script for ChestX-ray Dataset
...
実行例:
python gan_autoattack_eval.py --gan_ckpt ../checkpoints/best_model.pth --gpu 0
"""
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='Defense-GAN AutoAttack Evaluation')
...
class DefenseGANPurifier(nn.Module):
    """Defense-GAN: GANで再構成することで敵対的摂動を除去"""
    ...
    def find_best_z(self, x_gray):
        ...
        x_target = self.pixel_to_gan(x_gray)
...
def main():
    ...
    print(f"\n{'='*70}")
    print("AutoAttack + Defense-GAN Evaluation")
    print(f"{'='*70}")

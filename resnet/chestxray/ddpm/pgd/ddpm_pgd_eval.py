"""
ChestX-ray Dataset - PGD Attack + DDPM Purification Defense
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='ChestX-ray PGD Attack + DDPM Defense')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/chestxray/ddpm/pgd/results',
...
def run_pgd_attack(model, x_test, y_test, epsilon, alpha, steps, device, batch_size=32, random_start=True):
    ...
    print(f"\nRunning PGD attack...")
    print(f"  epsilon={epsilon:.4f}, alpha={alpha:.4f}, steps={steps}")
...
def main():
    ...
    with open(summary_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("ChestX-ray - PGD Attack + DDPM Defense\n")
        f.write("="*70 + "\n\n")
        f.write(f"Attack: PGD (L_inf)\n")

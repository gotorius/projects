"""
DermMel - AutoAttack + Guided-Diffusion (ImageNet Pretrained) Defense

ImageNet事前学習済み拡散モデルを用いた敵対的防御評価 (AutoAttack)
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='DermMel AutoAttack + Guided-Diffusion Defense')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/dermmel/imagenet/autoattack/results',
...
def load_classifier(args, device):
    ...
    # 正規化ラッパーでラップ
    wrapped_classifier = NormalizedClassifier(classifier, IMAGENET_MEAN, IMAGENET_STD).to(device).eval()
...
def main():
    ...
    classifier, wrapped_classifier = load_classifier(args, device)
    ...
    write_and_print(f"\n{'='*70}")
    write_and_print("AutoAttack + Guided-Diffusion (ImageNet) Defense Evaluation (DermMel)")

"""
DermMel - FGSM Attack + Guided-Diffusion (ImageNet Pretrained) Defense

ImageNet事前学習済み拡散モデルを用いた敵対的防御評価

実行例:
python imagenet_fgsm_eval.py --epsilon 0.031 --start_t 80 --t_purify 50
"""
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='DermMel FGSM Attack + Guided-Diffusion Defense')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/dermmel/imagenet/fgsm/results',
...
def load_classifier(args, device):
    ...
    classifier.fc = nn.Linear(num_features, 2)  # DermMel: 2クラス, Dropoutなし
...
def main():
    ...
    write_and_print(f"\n{'='*70}")
    write_and_print("FGSM Attack + Guided-Diffusion (ImageNet) Defense Evaluation (DermMel)")

"""
Defense-GAN Adversarial Defense Evaluation for DermMel Dataset - AutoAttack
...
python gan_autoattack_eval.py --epsilon 0.031 --version standard
python gan_autoattack_eval.py --epsilon 0.031 --rec_iters 200 --rec_rr 5
"""
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='Defense-GAN AutoAttack Evaluation (DermMel)')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/dermmel/gan/autoattack/results',
...
class NormalizedClassifier(nn.Module):
    """AutoAttack用に[0,1]入力をImageNet正規化して分類"""
...
def main():
    ...
    # AutoAttack用ラッパー
    wrapped_classifier = NormalizedClassifier(classifier, device).to(device).eval()
    ...
    log(f"\n{'='*70}")
    log("AutoAttack + Defense-GAN Evaluation (DermMel)")
    log(f"{'='*70}")
    log(f"Epsilon: {args.epsilon:.4f} ({args.epsilon*255:.1f}/255)")
    log(f"Norm: {args.norm}, Version: {args.version}")
    ...
    # 3. AutoAttack
    log(f"\n[3/4] AutoAttack ({args.version})...")
    start = time.time()
    
    adversary = AutoAttack(wrapped_classifier, norm=args.norm, eps=args.epsilon, 
    ...
    x_adv = adversary.run_standard_evaluation(
        x_test.to(device), y_test.to(device), bs=args.batch_size
    )
    x_adv = x_adv.cpu()

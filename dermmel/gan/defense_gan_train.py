"""
Defense-GAN: DermMel (皮膚メラノーマ) データセット用 訓練コード
...
特徴:
    ...
    6. 224x224 RGB画像に最適化
    7. Hinge lossの採用（安定した訓練）
    8. Orthogonal初期化

DermMelデータセット特有の考慮:
    - RGB (3チャンネル)
    - 皮膚病変画像
    - Melanoma / NotMelanoma の2クラス構成

Usage:
    python defense_gan_train.py --epochs 200 --batch_size 16 --gpu_id 0

メモリ使用量目安 (latent_dim=512):
    - batch_size=16: ~8GB VRAM
    - batch_size=8: ~5GB VRAM
    - batch_size=32: ~14GB VRAM
"""
...
# ========== 設定 ==========
def get_args():
    parser = argparse.ArgumentParser(description='Defense-GAN Training for DermMel')
    parser.add_argument('--data_dir', type=str,
                        default='/mnt/data1/Public/MedImages/DermMel',
                        help='訓練データのパス (train_sep/valid/test フォルダを含むディレクトリ)')
    ...
    parser.add_argument('--train_split', type=str, default='train_sep',
                        help='訓練データのサブフォルダ名 (train_sep or train)')
    return parser.parse_args()


# ========== DermMel Dataset ==========
class DermMelDataset(Dataset):
    """DermMel RGB データセット (ImageFolderベース)"""
    def __init__(self, root_dir, split='train_sep', img_size=224, augment=True):
...
class Generator(nn.Module):
    ...
    def __init__(self, latent_dim=512, ngf=64, nc=3):
        ...
        # Final convolution (output: 3 channels for RGB)
...
class Discriminator(nn.Module):
    """
    ResNet-based Discriminator for 224x224 RGB images with Self-Attention
...
def train(args):
    ...
    # Dataset (DermMel: RGB, 3 channels)
    print(f'\nLoading DermMel dataset from: {args.data_dir}')
    
    dataset = DermMelDataset(
        args.data_dir,
    ...
    if len(dataset) == 0:
        raise ValueError(f"No images found in {args.data_dir}/{args.train_split}. "

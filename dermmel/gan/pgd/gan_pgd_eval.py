"""
Defense-GAN Adversarial Defense Evaluation for DermMel Dataset - PGD Attack

PGD (Projected Gradient Descent) 攻撃:
- FGSMの反復版、より強力な攻撃
- 各ステップでεボール内に投影

実行例:
python gan_pgd_eval.py --epsilon 0.031 --use_defense
python gan_pgd_eval.py --epsilon 0.031 --pgd_steps 20 --pgd_alpha 0.007
python gan_pgd_eval.py --epsilon 0.031 --rec_iters 200 --rec_rr 5
"""
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='Defense-GAN PGD Evaluation (DermMel)')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/dermmel/gan/pgd/results',
...
class DefenseGAN:
    ...
    def _compute_loss(self, z, x_target):
        ...
        return F.mse_loss(x_gen, x_target)
    ...
    def _reconstruct_single(self, x_target):
        ...
        for r in range(self.rec_rr):
            ...
            if self.use_lbfgs:
                optimizer = torch.optim.LBFGS([z], lr=0.5, max_iter=20, line_search_fn='strong_wolfe')
    ...
    def reconstruct(self, x):
        batch_size = x.size(0)
        x_target = x * 2 - 1  # [0,1] -> [-1,1]
        
        reconstructed = []
        losses = []
        
        for i in range(batch_size):
            best_z, best_loss = self._reconstruct_single(x_target[i:i+1])
            losses.append(best_loss)
            
            with torch.no_grad():
                x_rec = self.generator(best_z)
                x_rec = (x_rec + 1) / 2  # [-1,1] -> [0,1]
...
def load_cached_samples(path):
    ...
    x_test = data['x_test']
    y_test = data['y_test']
    classes = data.get('classes', ['Melanoma', 'NotMelanoma'])
...
# ========== PGD攻撃 ==========
def pgd_attack(model, x, y, epsilon, alpha, steps, random_start, device):
    """PGD攻撃"""
    x = x.clone().to(device)
    x_adv = x.clone()
...
def save_sample_images(x_clean, x_adv, x_purified_clean, x_purified_adv, labels, classes, save_dir):
    ...
    save_image(make_grid(x_clean[:n], nrow=5, padding=2), os.path.join(save_dir, 'all_clean.png'))
    save_image(make_grid(x_adv[:n], nrow=5, padding=2), os.path.join(save_dir, 'all_adversarial.png'))
    
    if x_purified_clean is not None:
        save_image(make_grid(x_purified_clean[:n], nrow=5, padding=2), os.path.join(save_dir, 'all_purified_clean.png'))
    
    if x_purified_adv is not None:
        save_image(make_grid(x_purified_adv[:n], nrow=5, padding=2), os.path.join(save_dir, 'all_purified_adv.png'))
        
        comparison = []
        for i in range(min(5, n)):
            comparison.extend([x_clean[i], x_adv[i], x_purified_adv[i]])
        save_image(torch.stack(comparison), os.path.join(save_dir, 'comparison.png'), nrow=3, padding=2)
...
def main():
    ...
    log_dir = os.path.join(args.output_dir, f"{timestamp}_{defense_str}_{eps_str}_pgd")
    ...
    if args.use_defense:
        ...
        log(f"Defense-GAN: iters={args.rec_iters}, rr={args.rec_rr}, lbfgs={args.use_lbfgs}")
    ...
    log(f"\n{'='*70}")
    log("PGD Attack + Defense-GAN Evaluation (DermMel)")
    log(f"{'='*70}")
    log(f"Epsilon: {args.epsilon:.4f} ({args.epsilon*255:.1f}/255)")
    log(f"PGD steps: {args.pgd_steps}, alpha: {args.pgd_alpha:.4f}")
    ...
    for i in tqdm(range(0, len(x_test), args.batch_size), desc="PGD"):
        ...
        x_adv_batch = pgd_attack(
            classifier, x_batch, y_batch, args.epsilon, 
            args.pgd_alpha, args.pgd_steps, args.random_start, device
        )

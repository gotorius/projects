"""
DermMel (メラノーマ分類) - PGD攻撃 + DDPM浄化防御検証スクリプト
...
out_dir = f'/mnt/data1/gotou/projects/dermmel/ddpm/pdg/results_pgd{PGD_STEPS}_t{START_T}_s{PURIFY_STEPS}'
...
print(f"\n======================================")
print("Starting PGD + DDPM purification evaluation (DermMel)")
print("[Evaluation policy] Use only samples correctly classified by the clean model")
print("======================================")

for batch_idx, (x_norm, y) in enumerate(tqdm(valid_loader, desc='Eval (PGD->DDPM)')):
...
if total == 0:
    ...
    print("\n======================================")
    print("Results (DermMel PGD + DDPM)")
    print("======================================")
    ...
    with open(summary_txt, 'w') as f:
        f.write('=' * 70 + '\n')
        f.write('DermMel - PGD + DDPM Purification Summary\n')
        f.write('=' * 70 + '\n\n')

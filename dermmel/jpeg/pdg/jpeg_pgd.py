"""
DermMel (メラノーマ分類) - PGD攻撃 + JPEG圧縮防御検証スクリプト
ChestXray版 jpeg_pgd.py を皮膚病変データセットに適用
"""
import os
import io
import torch
...
out_dir = f'/mnt/data1/gotou/projects/dermmel/jpeg/pdg/results_q{JPEG_QUALITY}_pgd{PGD_STEPS}_a{int(PGD_ALPHA*255)}'
...
print(f"\n======================================")
print("Starting PGD + JPEG purification evaluation (DermMel)")
print("[Evaluation policy] Use only samples correctly classified by the clean model")
print("======================================")

for batch_idx, (x_norm, y) in enumerate(tqdm(valid_loader, desc='Eval (PGD->JPEG)')):
...
if total == 0:
    ...
    print("\n======================================")
    print("Results (DermMel PGD + JPEG)")
    print("======================================")
    ...
    with open(summary_txt, 'w') as f:
        f.write('=' * 70 + '\n')
        f.write('DermMel - PGD + JPEG Purification Summary\n')
        f.write('=' * 70 + '\n\n')

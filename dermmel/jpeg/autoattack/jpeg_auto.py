"""
DermMel (メラノーマ分類) - AutoAttack攻撃 + JPEG圧縮防御検証スクリプト
ChestXray版 jpeg_auto.py を皮膚病変データセットに適用
"""
import os
import io
import torch
...
print(f"\n======================================")
print("Starting AutoAttack + JPEG purification evaluation (DermMel)")
print("[Evaluation policy] Use only samples correctly classified by the clean model")
print("======================================")

for batch_idx, (x_norm, y) in enumerate(tqdm(valid_loader, desc='Eval (AutoAttack->JPEG)')):
...
if total == 0:
    ...
    print("\n======================================")
    print("Results (DermMel AutoAttack + JPEG)")
    print("======================================")
    ...
    with open(summary_txt, 'w') as f:
        f.write('=' * 70 + '\n')
        f.write('DermMel - AutoAttack + JPEG Purification Summary\n')
        f.write('=' * 70 + '\n\n')

"""
ChestXray (肺炎分類) - PGD攻撃 + JPEG圧縮防御検証スクリプト
ViT-B/16 分類器用
"""
import os
import io
...
print("Loading ViT classifier:", clf_ckpt)

# ViT-B/16 モデル構築
clf = models.vit_b_16(weights=None)
in_features = clf.heads.head.in_features
clf.heads.head = nn.Sequential(
    nn.Dropout(0.1),
...
clf = clf.to(device)
clf.eval()
print("ViT Classifier loaded. Best val acc:", ckpt.get('best_val_acc', 'N/A'))
...
@torch.no_grad()
def _predict(base_model, x_norm):
    logits = base_model(x_norm)
    return torch.argmax(logits, dim=1), logits

def run_pgd(base_model, x_norm, y, eps, alpha, steps, random_start=True):
    """
    base_model: 正規化入力想定の分類器
    x_norm: 正規化済み入力 (B,3,H,W)
    y: 正解ラベル
    eps: Linf半径 (ピクセル空間) 例: 8/255
    alpha: 1 step の更新幅 (ピクセル空間) 例: 2/255
    steps: 反復回数
    random_start: True のとき、[-eps, eps] の一様乱数で初期化

    return: adv_norm (正規化空間)
    """
    base_model.eval()

    x0_pix = denormalize(x_norm).detach()
    if random_start:
        x_adv = x0_pix + torch.empty_like(x0_pix).uniform_(-eps, eps)
    ...
    for _ in range(steps):
        ...
        logits = _forward_logits(base_model, x_adv)
        ...
        # Linf 投影
        x_adv = torch.max(torch.min(x_adv, x0_pix + eps), x0_pix - eps)
        # 画素域へ
        x_adv = torch.clamp(x_adv, 0.0, 1.0)

    adv_norm = renormalize(x_adv).detach()
    return adv_norm
...
out_dir = f'/mnt/data1/gotou/projects/chestxray/ViT/jpeg/pgd/results_q{JPEG_QUALITY}_pgd{PGD_STEPS}_a{int(PGD_ALPHA*255)}'
...
print(f"\n======================================")
print("Starting PGD + JPEG purification evaluation (ChestXray - ViT)")
print("[Evaluation policy] Use only samples correctly classified by the clean model")
print("======================================")

for batch_idx, (x_norm, y) in enumerate(tqdm(test_loader, desc='Eval (PGD->JPEG)')):
    ...
    # PGD adversarial (on filtered subset)
    adv_norm = run_pgd(clf, x_norm, y, EPSILON_PIXEL, PGD_ALPHA, PGD_STEPS, PGD_RANDOM_START)
    with torch.no_grad():
...
if total == 0:
    ...
    print("\n======================================")
    print("Results (ChestXray ViT PGD + JPEG)")
    print("======================================")
    ...
    print_cm(all_labels, all_adv, 'Adversarial (PGD)')
    ...
    with open(summary_txt, 'w') as f:
        f.write('=' * 70 + '\n')
        f.write('ChestXray ViT - PGD + JPEG Purification Summary\n')
        f.write('=' * 70 + '\n\n')
        f.write(f'Dataset: ChestXray test set (NORMAL vs PNEUMONIA)\n')
        f.write(f'Model: ViT-B/16\n')
        f.write(f'Attack: PGD, epsilon={EPSILON_PIXEL:.4f} ({EPSILON_PIXEL * 255:.1f}/255), steps={PGD_STEPS}, alpha={PGD_ALPHA:.4f} ({PGD_ALPHA*255:.1f}/255), random_start={PGD_RANDOM_START}\n')

...
class SimpleUNet(nn.Module):
    def __init__(self, in_ch=1, base_ch=64, time_emb_dim=256):  # 1チャンネル用に変更
        super().__init__()
        self.time_mlp = nn.Sequential(
...
ddpm = SimpleUNet(in_ch=1, base_ch=64, time_emb_dim=256).to(device)  # 1チャンネル用に変更
...
start_t = 80    # 浄化開始時刻
T_purify = 50    # 逆拡散反復数
save_examples_dir = "/mnt/data1/gotou/projects/chestxray/ddpm/pgd/purify_examples2"
...
for batch_idx, (images_norm, labels) in enumerate(tqdm(test_loader, desc="Eval loop (PGD)")):
    ...
    with torch.no_grad():
        ...
        # 浄化後のノルム計算（グレースケール→RGBに変換して比較）
        pur_pixel_gray = (purified_minus1to1.detach() + 1.0) / 2.0  # [-1,1] -> [0,1]
        pur_pixel_gray_resized = F.interpolate(pur_pixel_gray, size=(224,224), mode="bilinear", align_corners=False)
        pur_pixel_resized = grayscale_to_rgb_pixel(pur_pixel_gray_resized)  # [B,1,H,W] -> [B,3,H,W]
    ...
    if saved_image_count < MAX_IMAGES_TO_SAVE:
        ...
        pur_pixel_save_gray = (purified_minus1to1.detach() + 1.0) / 2.0
        pur_pixel_save_rgb = grayscale_to_rgb_pixel(pur_pixel_save_gray)
        ...
        pur_resized = F.interpolate(pur_pixel_save_rgb, size=(224,224), mode="bilinear", align_corners=False)

        for i in range(len(correct_indices)):
            if saved_image_count >= MAX_IMAGES_TO_SAVE:
                break

            # triplet tile のみ保存（labelは未使用）

...
OUT_DIR = os.path.join('/mnt/data1/gotou/projects/chestxray', 'ddpm_out3')
...
# 修正: 回転を削除し、より保守的な拡張に変更（傾き問題の解消）
train_transform = transforms.Compose([
    transforms.Grayscale(num_output_channels=1),
    transforms.RandomResizedCrop(IMAGE_SIZE, scale=(0.90, 1.0)),  # 0.85 → 0.90
    transforms.RandomHorizontalFlip(p=0.3),  # 0.5 → 0.3 に削減
    # transforms.RandomRotation(degrees=7, fill=0),  # 削除: 傾き問題の原因
    transforms.RandomAutocontrast(p=0.3),  # 0.2 → 0.3
    # RandomAdjustSharpness 削除: 過度な加工を避ける
    transforms.ToTensor(),
])
...
def p_sample(model, x_t, t):
    ...
    # posterior variance は事前計算を使用（過度なノイズ抑制のため80%に削減）
    posterior_var_t = posterior_variance[t].view(-1, 1, 1, 1).clamp(min=1e-20)
    posterior_var_t = posterior_var_t * 0.8  # ノイズ削減で霧を軽減
...
def post_process_samples(samples):
    """
    DDPMサンプリング後の品質向上処理（医療画像特化）
    白い靄を軽減し、コントラストを改善
    """
    # [-1, 1] から [0, 1] に変換
    samples = (samples + 1.0) / 2.0
    
    # 1. パーセンタイルクリッピング（外れ値除去で霧を軽減）
    p_low = torch.quantile(samples, 0.02)
    p_high = torch.quantile(samples, 0.98)
    samples = (samples - p_low) / (p_high - p_low + 1e-8)
    ...
    for i in range(samples.size(0)):
        ...
        if img_std > 1e-6:
            # 適度な正規化でコントラストを改善
            samples[i] = (img - img_mean) / (img_std + 1e-8) * 0.2 + 0.5
            samples[i] = torch.clamp(samples[i], 0.0, 1.0)
...
def p_sample_loop(model, shape, device):
    ...
    # 後処理で品質向上（DDPMサンプリング後）
    img = post_process_samples(img)
    return img
...
if __name__ == "__main__":
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if need_sample:
            ...
            # samples は post_process_samples 内で既に [0,1] に変換済み

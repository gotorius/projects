"""
ChestXray (肺炎分類) - AutoAttack攻撃 + DDPM浄化防御検証スクリプト
PCam版 ddpm_auto.py を胸部X線データセットに適用
"""
import os
import torch
...
start_t=80; purify_steps=50
out_dir = '/mnt/data1/gotou/projects/chestxray/ddpm/autoattack/purify_examples'
...
# 統計
all_labels=[]; all_clean=[]; all_adv=[]; all_pur=[]
correct_clean=0; correct_adv=0; correct_pur=0; total=0
l2_adv=[]; linf_adv=[]; l2_pur=[]; linf_pur=[]

print(f"\n======================================")
print("Starting AutoAttack + DDPM purification evaluation (ChestXray)")
print("[Evaluation policy] Use only samples correctly classified by the clean model")
print("======================================")

for batch_idx,(x_norm,y) in enumerate(tqdm(test_loader, desc='Eval (AutoAttack)')):
...
print("\n======================================")
print("Results (ChestXray AutoAttack + DDPM)")
print("======================================")
...
with open(summary_txt,'w') as f:
    f.write('='*70+'\n')
    f.write('ChestXray - AutoAttack + DDPM Purification Summary\n')
    f.write('='*70+'\n\n')

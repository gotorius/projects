"""
VAE v3 (MagNet-style) Training Script for ChestX-ray Dataset
...
8. Self-Attention GAN: "Self-Attention Generative Adversarial Networks"
   - Zhang et al., ICML 2019
   - https://arxiv.org/abs/1805.08318

9. Cyclical Annealing: "Cyclical Annealing Schedule: A Simple Approach to Mitigating KL Vanishing"
   - Fu et al., NAACL 2019
   - https://arxiv.org/abs/1903.10145

10. Feature Matching: "Improved Techniques for Training GANs"
...
v3 改善点 (v2からの変更):
============================================================
1. メモリ効率化: gradient checkpointing, mixed precision対応
2. パラメータ調整: 再構成品質を重視した重み設定
3. Warm restart付きCosine Annealing
4. 学習率ウォームアップ
5. より強いPerceptual Loss重み
6. 定期的なメモリ解放

実行例:
python train_vae_v3.py --epochs 300 --batch_size 8 --gpu 0

メモリ目安:
- batch_size=8: ~4-5GB VRAM
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='Train VAE v3 for ChestX-ray')
    
    # モデル設定
    parser.add_argument('--latent_dim', type=int, default=256,
                        help='Latent space dimension (smaller for stability)')
    parser.add_argument('--base_ch', type=int, default=48,
                        help='Base number of channels (reduced for memory)')
    ...
    parser.add_argument('--lambda_perceptual', type=float, default=1.0,
                        help='Perceptual loss weight (higher for sharpness)')
    parser.add_argument('--lambda_adv', type=float, default=0.05,
                        help='Adversarial loss weight (lower for stability)')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/chestxray/vae/checkpoints_v3',
...
# ========== Perceptual Loss (メモリ効率化版) ==========
class PerceptualLoss(nn.Module):
    """Memory-efficient VGG Perceptual Loss"""
...
class Discriminator(nn.Module):
    """Lightweight Discriminator for memory efficiency"""
...
def train(args):
    ...
    disc = Discriminator(img_channels=1, base_ch=32).to(device)
    ...
    history = {
        ...
        'perceptual_loss': [], 'ssim_loss': [], 'disc_loss': []
    }
    ...
    print(f"\n{'='*70}")
    print(f"Starting VAE v3 training for {args.epochs} epochs...")
    ...
    print(f"Loss weights: β_max={args.beta_max}, perc={args.lambda_perceptual}, "
          f"adv={args.lambda_adv}, ssim={args.lambda_ssim}")
    print(f"{'='*70}\n")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        print(f"Epoch {epoch}: total={epoch_losses['total_loss']:.4f}, "
              ...
              f"ssim={epoch_losses['ssim_loss']:.4f}, lr={current_lr:.6f}")

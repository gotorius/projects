"""
ChestX-ray Dataset - FGSM Attack + Guided-Diffusion (ImageNet Pretrained) Defense
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='ChestX-ray FGSM Attack + Guided-Diffusion Defense')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/chestxray/imagenet/fgsm/results',
...
# ========== FGSM攻撃実行 ==========
def run_fgsm_attack(model, x_test, y_test, epsilon, device, batch_size=8):
...
def main():
    ...
    # ==================== 評価開始 ====================
    print(f"\n{'='*70}")
    print("FGSM Attack + Guided-Diffusion (ImageNet) Defense Evaluation")
    print(f"{'='*70}")
    ...
    with open(summary_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("ChestX-ray - FGSM Attack + Guided-Diffusion (ImageNet) Defense\n")
        f.write("="*70 + "\n\n")

...
OUT_DIR = os.path.join('/mnt/data1/gotou/kaggle/chestxray', 'ddpm_out')
...
# ChestXray向け前処理: 大域構造保持のためResize+CenterCrop
# 医学画像のため、反転は除外（左右の肺の位置情報が重要）
train_transform = transforms.Compose([
    transforms.Resize(IMAGE_SIZE + 32),
    transforms.CenterCrop(IMAGE_SIZE),
    # RandomHorizontalFlip は除外（X線画像では左右の位置情報が医学的に重要）
    transforms.ToTensor(),
])

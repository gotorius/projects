"""
ChestX-ray Dataset - AutoAttack + JPEG Compression Defense
...
4. AutoAttack敵対的画像をJPEG圧縮した後の分類精度（防御あり）
"""

"""
# 基本実行（デフォルト設定: quality=11）
python jpeg_autoattack_eval.py
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='ChestX-ray AutoAttack + JPEG Defense')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/chestxray/jpeg/autoattack/results',
...
# ========== AutoAttack実行 ==========
def run_autoattack(model, x_test, y_test, epsilon, version, device, batch_size=32):
...
def main():
    ...
    # ==================== 評価開始 ====================
    print(f"\n{'='*70}")
    print("AutoAttack + JPEG Compression Defense Evaluation")
    print(f"{'='*70}")
    ...
    with open(summary_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("ChestX-ray - AutoAttack + JPEG Compression Defense\n")
        f.write("="*70 + "\n\n")

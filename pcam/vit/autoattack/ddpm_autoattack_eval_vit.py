"""
DDPM-based Adversarial Defense Evaluation for PCam Dataset - AutoAttack (ViT Classifier)

ViT分類器を使用したDDPM防御評価スクリプト

実行例:
python ddpm_autoattack_eval_vit.py --epsilon 0.031 --version standard
"""
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='DDPM Defense Evaluation - AutoAttack (ViT)')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/pcam/vit/autoattack/results',
...
def main():
    ...
    write_and_print(f"\n{'='*70}")
    write_and_print("AutoAttack + DDPM Defense Evaluation (ViT Classifier)")
    ...
    # 最終結果
    write_and_print(f"\n{'='*70}")
    write_and_print("FINAL RESULTS (ViT Classifier)")
    write_and_print(f"{'='*70}")
    write_and_print(f"Classifier: ViT-B/16")
    write_and_print(f"Attack: AutoAttack ({args.version}), Epsilon: {args.epsilon:.4f} ({args.epsilon*255:.1f}/255)")

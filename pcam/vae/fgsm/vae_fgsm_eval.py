"""
VAE (MagNet-style) FGSM Evaluation Script for PCam (PatchCamelyon) Dataset
...
実行例:
python vae_fgsm_eval.py --vae_ckpt ../checkpoints_v2/20260108_165711/best_model.pth --gpu 0
"""
...
# ========== 引数パーサー ==========
def parse_args():
    parser = argparse.ArgumentParser(description='VAE (MagNet) FGSM Evaluation for PCam')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/pcam/vae/fgsm/results',
...
def main():
    ...
    print(f"\n{'='*70}")
    print("FGSM Attack + VAE (MagNet) Defense Evaluation for PCam")
    print(f"{'='*70}")
    ...
    with open(os.path.join(log_dir, 'results.txt'), 'w') as f:
        f.write("="*70 + "\n")
        f.write("FGSM Attack + VAE (MagNet) Defense Evaluation Results for PCam\n")
        f.write("="*70 + "\n\n")

...
# AutoAttack用のモデルラッパー（正規化を含む）
class NormalizedModel(nn.Module):
    def __init__(self, model, mean, std):
        super().__init__()
        self.model = model
        # mean, std を [1, C, 1, 1] の形状で保持
        if mean.ndim == 1:
...
def autoattack_attack(model, images, labels, epsilon_pixel, device,
    ...
    # NormalizedModelを作成（mean/stdは [1,C,1,1] または [C] の形状）
...
# loop
for batch_idx, (images_norm, labels) in enumerate(tqdm(EVAL_LOADER, desc="Eval loop with AutoAttack (all validation images)")):
    ...
    # 2) adversarial example (AutoAttack) - 正解した画像のみ
    adv_images_norm, adv_preds_from_attack = autoattack_attack(
        model=clf,
...
print("\n" + "="*70)
print("==== Results (Clean Images Only - AutoAttack) ====")
print("="*70)
print(f"Total samples evaluated: {total} (元画像で正解したもののみ)")
print(f"Attack: AutoAttack with epsilon={epsilon_pixel:.4f} ({epsilon_pixel*255:.1f}/255)")
print(f"Purification: DDPM start_t={start_t}, T_purify={T_purify}")
print("-"*70)
print(f"Clean accuracy:     {clean_acc:.4f} (常に1.0)")
print(f"Adv (AutoAttack) accuracy:{adv_acc:.4f}")
...
with open(summary_path, 'w') as f:
    f.write("="*70 + "\n")
    f.write("AutoAttack + DDPM Purification - Summary Statistics\n")
    f.write("="*70 + "\n\n")
    f.write(f"Attack Parameters:\n")
    f.write(f"  Method: AutoAttack (ensemble of attacks)\n")

"""
Defense-GAN Adversarial Defense Evaluation for PCam Dataset - PGD Attack (v4 Improved)
...
実行例:
python gan_pgd_eval_v4.py --epsilon 0.031 --use_defense
python gan_pgd_eval_v4.py --epsilon 0.031 --pgd_steps 20 --pgd_alpha 0.007
python gan_pgd_eval_v4.py --epsilon 0.031 --rec_iters 200 --rec_rr 5
"""
...
# ========== 引数 ==========
def parse_args():
    parser = argparse.ArgumentParser(description='Defense-GAN PGD Evaluation v4')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/data1/gotou/projects/pcam/gan/pgd/results_v4')
...
def main():
    ...
    if args.pgd_alpha is None:
        args.pgd_alpha = args.epsilon / 4  # Default step size
    
    # 出力ディレクトリ
    timestamp = datetime.now().strftime("%m%d%H%M")
    defense_str = "defense_v4" if args.use_defense else "no_defense"
    eps_str = f"eps{int(args.epsilon*255)}"
    log_dir = os.path.join(args.output_dir, f"{timestamp}_{defense_str}_{eps_str}_pgd{args.pgd_steps}")
    ...
    log(f"\n{'='*70}")
    log("PGD Attack + Defense-GAN v4 Evaluation (PCam)")
    log(f"{'='*70}")
    log(f"Epsilon: {args.epsilon:.4f} ({args.epsilon*255:.1f}/255)")
    log(f"PGD Steps: {args.pgd_steps}")
    log(f"PGD Alpha: {args.pgd_alpha:.4f} ({args.pgd_alpha*255:.1f}/255)")
    log(f"Random Start: {args.random_start}")
    ...
    for i in tqdm(range(0, len(x_test), args.batch_size), desc="PGD"):
        ...
        x_adv_batch = pgd_attack(
            ...
            args.epsilon, args.pgd_alpha, args.pgd_steps,
            args.random_start, device
        )
    ...
    cm_results['adv'] = print_confusion_matrix(y_true, pred_adv, "Adversarial (PGD)", classes, results_file)
    if args.use_defense:
